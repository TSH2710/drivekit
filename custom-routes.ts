...
app.post('/shopify/update-variants', requireAdminMiddleware, async (c) => {
  ...
  try {
    ...
    const productData = await restGet(`/products/${productId}.json`)
    const product = productData.product
    if (!product) return c.json({ error: 'Product not found' }, 404)

    // Step 2: Delete ALL existing variants (REST API: delete one by one)
    for (const v of product.variants ?? []) {
      try {
        await restDelete(`/products/${productId}/variants/${v.id}.json`)
        steps.push(`deleted variant "${v.title}"`)
      } catch (e: any) {
        steps.push(`skip delete variant: ${e.message}`)
      }
      await new Promise(r => setTimeout(r, 200))
    }

    // Step 3: Update product with new options and first variant
    // Shopify REST: PUT product with options and variants creates them
    const firstVariant = variants[0]
    const productUpdate: any = {
      id: parseInt(productId),
      options: options.map(o => ({ name: o.name, values: o.values })),
      variants: [{
        option1: firstVariant.optionValues[options[0]?.name ?? 'Color'] ?? firstVariant.optionValues[Object.keys(firstVariant.optionValues)[0]],
        option2: options.length > 1 ? (firstVariant.optionValues[options[1]?.name ?? 'Quantity'] ?? firstVariant.optionValues[Object.keys(firstVariant.optionValues)[1]]) : undefined,
        price: firstVariant.price,
        sku: firstVariant.sku,
        inventory_policy: 'deny',
      }],
    }
    await restPut(`/products/${productId}.json`, { product: productUpdate })
    steps.push(`updated product options: ${options.map(o => o.name).join(' + ')}`)
    ...
    for (let i = 1; i < variants.length; i++) {
      ...
      const variantData: any = {
        ...
        sku: v.sku,
        inventory_policy: 'deny',
        option1: v.optionValues[options[0]?.name ?? 'Color'] ?? v.optionValues[Object.keys(v.optionValues)[0]],
        option2: options.length > 1 ? (v.optionValues[options[1]?.name ?? 'Quantity'] ?? v.optionValues[Object.keys(v.optionValues)[1]]) : undefined,
      }
      await restPost(`/products/${productId}/variants.json`, { variant: variantData })
      steps.push(`created variant: ${Object.values(v.optionValues).join(' / ')} - $${v.price}`)
...
app.get('/shopify/retag-batches', requireAdminMiddleware, async (c) => {
  try {
    ...
    for (const p of slice) {
      ...
      const batchNum = Math.floor(globalIdx / BATCH_SIZE) + 1
      const batchTag = `Batch${batchNum}`
      const oldTags: string[] = Array.isArray(p.tags) ? p.tags : (typeof p.tags === 'string' ? p.tags.split(',').map((t: string) => t.trim()) : [])
    ...
    products.forEach((_: any, i: number) => { const b = Math.floor(i / BATCH_SIZE); if (b < 3) batchCounts[b]++ })
...
app.post('/shopify/upload-cover', requireAdminMiddleware, async (c) => {
  ...
  try {
    ...
    return c.json({
      ok: true,
      image: { id: newImage.id, src: newImage.src, position: newImage.position },
      message: `Cover image uploaded for product ${productId}`,
    })
  } catch (err: any) {
  ...
  ;(async () => {
    try {
      for (const cjTitle of cjTitles) {
        ...
        // Strategy: Create ALL variants (existing + new) in a single PUT call
        try {
          // Build array of all variants to include in the update
          ...
          const newVariantPayloads = variantsToCreate.map((v: string) => ({
            option1: v,
            price: '6.00',
          }))
          const allVariants = [...existingVariantPayloads, ...newVariantPayloads]

          // Also merge option values to include new variant titles
          const productOptions = shopifyProduct.options ?? []
          const mergedOptions = productOptions.map((opt: any) => ({
            name: opt.name,
            values: [...new Set([...opt.values, ...variantsToCreate])],
          }))
          ...
          if (res.ok) {
            ...
            errors.push(`PUT failed: ${res.status} — ${t.slice(0, 300)}`)
          }
